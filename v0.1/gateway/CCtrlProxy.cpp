...
int CCtrlProxy::generateKey(CGroup &group)
{
	...
	if (deriveGroup(*CGwData::getInstance()->token_, group.hGroup_, CGwData::getInstance()->hGw_, salt, sizeof(salt)) != 0)
	...
	for (std::list<CSe>::iterator it = group.seList_.begin(); it != group.seList_.end(); it++) {
		...
		if (deriveGroup(*CGwData::getInstance()->token_, it->h_, group.hGroup_, salt, sizeof(salt)) != 0)
